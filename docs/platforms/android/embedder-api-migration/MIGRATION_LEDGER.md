...
|---|---|---|---|---|
| **VD** — Virtual Display | `legacy` | `platform_view/virtual_display_platform_view_main.dart`; blocked by **B-5** | _TBD T-0.12_ | — |
| **TLHC** — Texture Layer HC *(default)* | `legacy` | `platform_view/texture_layer_hybrid_composition_platform_view_main.dart` | _TBD T-0.12_ | — |
| **HC** — Hybrid Composition | `legacy` | `platform_view/hybrid_composition_platform_view_main.dart`, `hybrid_android_views_integration_test`; gated on **T-1.18** (thread merging API extension) | _TBD T-0.12_ | — |
| **HCPP** *(Impeller Vulkan + API 34 only)* | `legacy` | `hcpp/*` (14 mains, Vulkan); GLES fallback test (T-0.11) | _TBD T-0.12_ | — |
| **SurfaceTexture** ext. texture | `legacy` | `external_texture/surface_texture_smiley_face_main.dart`; blocked by **B-5** | _TBD T-0.12_ | — |
| **SurfaceProducer/ImageReader** ext. texture | `legacy` | `external_texture/surface_producer_smiley_face_main.dart` | _TBD T-0.12_ | — |
