...
Multiple previous LLM migration attempts have been analyzed, and this plan synthesizes their successes while directly preventing their recurrent architectural mistakes. Adhere to this document strictly to ensure ABI stability, subsystem completeness, and zero-debt code cleanup.
...
### 3. JNI Conditional Dispatch (Transition Phase)
...
### 4. Zero-Debt Surgical Cleanup (Phase 5)
When the feature flag is rolled out (Phase 5.2), your final cleanup branch must be absolute.
* Delete the `android_legacy_engine_holder` GN target.
* Delete `AndroidShellHolder`, `PlatformViewAndroid`, legacy adapters, and old EGL/Surface wrapper implementations.
* Prune prohibited dependencies (`flow`, `runtime`, `shell/common`, `skia`, `impeller`, `lib/ui`, `txt`, `assets`) from the Android `BUILD.gn`.
...
* **2.1**: Break `PlatformViewAndroid` Inheritance & Establish GN Quarantine Target
...
* **5.2**: Legacy Bridge Removal & Total BUILD.gn Dependency Pruning (100% deletion of previous architecture).
