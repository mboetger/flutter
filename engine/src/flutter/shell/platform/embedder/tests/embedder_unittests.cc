...
TEST_F(EmbedderTest, RegisterImageDecoderValidation) {
  ...
  EXPECT_EQ(
      FlutterEngineRegisterImageDecoder(
          nullptr,
          [](const uint8_t* data, size_t size, void* user_data) { return true; },
          nullptr, 0),
      kInvalidArguments);
  ...
  auto decoder_cb = [](const uint8_t* data, size_t size, void* user_data) -> bool {
...
TEST_P(EmbedderTestMultiBackend, CanLaunchAndShutdown) {
...
TEST_P(EmbedderTestMultiBackend, CanInvokeCustomEntrypoint) {
  auto& context = GetEmbedderContext();
  fml::AutoResetWaitableEvent latch;
...
TEST_P(EmbedderTestMultiBackend, CanSendPointerAndWindowMetrics) {
  auto& context = GetEmbedderContext();
  fml::AutoResetWaitableEvent latch;
  context.AddIsolateCreateCallback([&latch]() { latch.Signal(); });
  EmbedderConfigBuilder builder(context);
...
TEST_P(EmbedderTestMultiBackend, CanRegisterAndUnregisterExternalTexture) {
  auto& context = GetEmbedderContext();
  fml::AutoResetWaitableEvent latch;
  context.AddIsolateCreateCallback([&latch]() { latch.Signal(); });
  EmbedderConfigBuilder builder(context);
