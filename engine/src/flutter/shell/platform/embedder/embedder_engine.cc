...
EmbedderEngine::EmbedderEngine(
    ...
    : thread_host_(std::move(thread_host)),
      task_runners_(task_runners),
      run_configuration_(
          RunConfiguration::InferFromSettings(shell->GetSettings())),
      shell_args_(nullptr),
...
void EmbedderEngine::CollectThreadHost() {
  ...
  // Only invalidate runners and block UI thread if this is the last reference
  // to the shared thread host.
