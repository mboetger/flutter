...
class EmbedderWrappedTextureSourceVK : public impeller::TextureSourceVK {
 public:
  ...
  ~EmbedderWrappedTextureSourceVK() override {
 ...
 private:
  ...
  impeller::vk::ImageView GetRenderTargetView(
    ...
    // Backing store images are always a single 2D mip and layer.
    return image_view_.get();
  }
...
MakeRenderTargetFromBackingStoreImpeller(
  ...
  if (!vulkan) {
    FML_LOG(ERROR) << "Embedder supplied null Vulkan backing store.";
    return nullptr;
  }
  if (vulkan->struct_size != sizeof(FlutterVulkanBackingStore)) {
    FML_LOG(ERROR)
        << "Embedder supplied Vulkan backing store with invalid struct size: "
        << vulkan->struct_size;
    return nullptr;
  }
  if (!vulkan->image) {
  ...
  if (vulkan->image->struct_size != sizeof(FlutterVulkanImage)) {
    FML_LOG(ERROR)
        << "Embedder supplied Vulkan image with invalid struct size: "
        << vulkan->image->struct_size;
    return nullptr;
  }
  if (vulkan->image->image == 0) {
    FML_LOG(ERROR) << "Embedder supplied invalid Vulkan image handle (0).";
    return nullptr;
  }
  if (config.size.width <= 0 || config.size.height <= 0) {
    FML_LOG(ERROR) << "Embedder supplied invalid backing store dimensions: "
                   << config.size.width << "x" << config.size.height;
    return nullptr;
  }

  if (!aiks_context) {
    FML_LOG(ERROR) << "AiksContext was null.";
    return nullptr;
  }
  auto context = aiks_context->GetContext();
  if (!context) {
    FML_LOG(ERROR) << "AiksContext had null Context.";
    return nullptr;
  }

  impeller::ContextVK& context_vk = impeller::ContextVK::Cast(*context);
  ...
  impeller::TextureDescriptor resolve_tex_desc;
  resolve_tex_desc.type = impeller::TextureType::kTexture2D;
  resolve_tex_desc.format = format.value();
  resolve_tex_desc.size = size;
  resolve_tex_desc.storage_mode = impeller::StorageMode::kDevicePrivate;
  // 1 mip level and 1 sample for the resolve/onscreen render target texture.
  ...
  view_info.format = ToVKImageFormat(resolve_tex_desc.format);
  view_info.subresourceRange.aspectMask =
      impeller::vk::ImageAspectFlagBits::eColor;
  // 0th mip level and 0th array layer.
  ...
  if (result != impeller::vk::Result::eSuccess) {
    FML_LOG(ERROR) << "Failed to create Vulkan image view for provided image: "
                   << impeller::vk::to_string(result);
    return nullptr;
  }

  fml::closure image_destruct = [callback = vulkan->destruction_callback,
  ...
  auto wrapped_texture_source =
      std::make_shared<EmbedderWrappedTextureSourceVK>(
          vk_image, std::move(image_view), resolve_tex_desc,
          std::move(image_destruct));
  ...
  if (enable_msaa) {
    ...
    msaa_tex_desc.storage_mode = impeller::StorageMode::kDeviceTransient;
    msaa_tex_desc.type = impeller::TextureType::kTexture2DMultisample;
    // 4x MSAA count for antialiasing.
    msaa_tex_desc.sample_count = impeller::SampleCount::kCount4;
    msaa_tex_desc.format = resolve_tex->GetTextureDescriptor().format;
    msaa_tex_desc.size = size;
    msaa_tex_desc.usage = impeller::TextureUsage::kRenderTarget;
    ...
    auto msaa_tex =
        context->GetResourceAllocator()->CreateTexture(msaa_tex_desc);
    if (!msaa_tex) {
      FML_LOG(ERROR) << "Could not allocate Vulkan MSAA color texture.";
      return nullptr;
    }
    msaa_tex->SetLabel("ImpellerBackingStoreColorMSAA");
...
FlutterEngineResult FlutterEngineInitialize(size_t version,
  ...
  const auto* custom_asset_resolvers =
      SAFE_ACCESS(args, asset_resolvers, nullptr);
  const size_t custom_asset_resolvers_count =
      SAFE_ACCESS(args, asset_resolvers_count, 0);

  if (SAFE_ACCESS(args, assets_path, nullptr) == nullptr &&
      (custom_asset_resolvers == nullptr ||
       custom_asset_resolvers_count == 0)) {
    return LOG_EMBEDDER_ERROR(
        kInvalidArguments,
        "The assets path in the Flutter project arguments was missing.");
  }

  std::vector<std::unique_ptr<flutter::EmbedderAssetResolver>>
      embedder_resolvers;
  if (custom_asset_resolvers != nullptr && custom_asset_resolvers_count > 0) {
    embedder_resolvers.reserve(custom_asset_resolvers_count);
    for (size_t i = 0; i < custom_asset_resolvers_count; ++i) {
      if (custom_asset_resolvers[i] == nullptr) {
        return LOG_EMBEDDER_ERROR(
            kInvalidArguments,
            "An asset resolver in FlutterProjectArgs.asset_resolvers was "
            "null.");
      }
      if (custom_asset_resolvers[i]->struct_size !=
          sizeof(FlutterAssetResolver)) {
        return LOG_EMBEDDER_ERROR(
            kInvalidArguments,
            "An asset resolver in FlutterProjectArgs.asset_resolvers had an "
            "invalid struct size.");
      }
      if (custom_asset_resolvers[i]->get_asset_callback == nullptr) {
        return LOG_EMBEDDER_ERROR(
            kInvalidArguments,
            "An asset resolver in FlutterProjectArgs.asset_resolvers had a "
            "null "
            "get_asset_callback.");
      }
      embedder_resolvers.push_back(
          std::make_unique<flutter::EmbedderAssetResolver>(
              *custom_asset_resolvers[i]));
    }
  }
  ...
  if (!flutter::DartVM::IsRunningPrecompiledCode()) {
    // Verify the assets path or custom asset resolvers contain Dart 2 kernel
    // assets.
    ...
    if (!kernel_found && !embedder_resolvers.empty()) {
      for (const auto& resolver : embedder_resolvers) {
        if (resolver->GetAsMapping(kApplicationKernelSnapshotFileName) !=
            nullptr) {
          kernel_found = true;
          break;
        }
    ...
    if (SAFE_ACCESS(vulkan_config, external_texture_frame_callback, nullptr)) {
      external_texture_vk_callback =
          [ptr = vulkan_config->external_texture_frame_callback, user_data](
              int64_t texture_identifier, size_t width,
              size_t height) -> std::unique_ptr<FlutterVulkanImage> {
        std::unique_ptr<FlutterVulkanImage> image =
            std::make_unique<FlutterVulkanImage>();
        image->struct_size = sizeof(FlutterVulkanImage);
        if (!ptr(user_data, texture_identifier, width, height, image.get())) {
          return nullptr;
        }
        return image;
      };
  ...
  for (auto& resolver : embedder_resolvers) {
    asset_manager->PushBack(std::move(resolver));
  }

  auto run_configuration =
      flutter::RunConfiguration::InferFromSettings(settings, asset_manager);
...
FlutterEngineResult FlutterEngineSpawn(FLUTTER_API_SYMBOL(FlutterEngine) engine,
  ...
  if (!info) {
    return LOG_EMBEDDER_ERROR(kInvalidArguments, "Spawn info was invalid.");
  }

  if (info->struct_size < sizeof(FlutterEngineSpawnInfo)) {
    return LOG_EMBEDDER_ERROR(
        kInvalidArguments,
        "FlutterEngineSpawnInfo struct size was smaller than expected.");
  }
  ...
  if (!parent_engine->IsValid()) {
    return LOG_EMBEDDER_ERROR(kInvalidArguments,
                              "Parent engine is not valid or not running.");
  }

  const auto& parent_shell = parent_engine->GetShell();
  const auto& settings = parent_shell.GetSettings();

  const FlutterRendererConfig* config =
      SAFE_ACCESS(info, renderer_config, nullptr);
  if (!config) {
    return LOG_EMBEDDER_ERROR(
        kInvalidArguments,
        "Renderer configuration must be provided in FlutterEngineSpawnInfo.");
  }
  ...
  const FlutterProjectArgs* args = SAFE_ACCESS(info, project_args, nullptr);
  void* user_data = SAFE_ACCESS(info, user_data, nullptr);

  // Set up platform dispatch table for the spawned engine.
  flutter::PlatformViewEmbedder::UpdateSemanticsCallback
  ...
  flutter::PlatformViewEmbedder::RequestDartDeferredLibraryCallback
      request_dart_deferred_library_callback = nullptr;
  const FlutterCompositor* compositor = nullptr;
  ...
  if (args != nullptr) {
    ...
    if (SAFE_ACCESS(args, on_pre_engine_restart_callback, nullptr) != nullptr) {
      on_pre_engine_restart_callback =
          [ptr = args->on_pre_engine_restart_callback, user_data]() {
            return ptr(user_data);
          };
    }

    if (SAFE_ACCESS(args, channel_update_callback, nullptr) != nullptr) {
      channel_update_callback = [ptr = args->channel_update_callback,
    ...
    compositor = SAFE_ACCESS(args, compositor, nullptr);
  }

  auto external_view_embedder_result =
      InferExternalViewEmbedderFromArgs(compositor, settings.enable_impeller);
  if (!external_view_embedder_result.ok()) {
    FML_LOG(ERROR) << external_view_embedder_result.status().message();
    return LOG_EMBEDDER_ERROR(kInvalidArguments,
                              "Compositor arguments were invalid.");
  ...
  // Handle custom asset resolvers if provided in project_args.
  std::shared_ptr<flutter::AssetManager> asset_manager;
  if (args && SAFE_ACCESS(args, asset_resolvers, nullptr) != nullptr &&
      SAFE_ACCESS(args, asset_resolvers_count, 0) > 0) {
    asset_manager = std::make_shared<flutter::AssetManager>();
    size_t count = SAFE_ACCESS(args, asset_resolvers_count, 0);
    const FlutterAssetResolver* const* resolvers =
        SAFE_ACCESS(args, asset_resolvers, nullptr);
    for (size_t i = 0; i < count; ++i) {
      if (resolvers[i] == nullptr) {
        return LOG_EMBEDDER_ERROR(
            kInvalidArguments,
            "An asset resolver in FlutterProjectArgs.asset_resolvers was "
            "null.");
      }
      if (resolvers[i]->struct_size != sizeof(FlutterAssetResolver)) {
        return LOG_EMBEDDER_ERROR(
            kInvalidArguments,
            "An asset resolver in FlutterProjectArgs.asset_resolvers had an "
            "invalid struct size.");
      }
      if (resolvers[i]->get_asset_callback == nullptr) {
        return LOG_EMBEDDER_ERROR(
            kInvalidArguments,
            "An asset resolver in FlutterProjectArgs.asset_resolvers had a "
            "null get_asset_callback.");
      }
      asset_manager->PushBack(
          std::make_unique<flutter::EmbedderAssetResolver>(*resolvers[i]));
    }
  }

  flutter::RunConfiguration run_configuration =
      asset_manager ? flutter::RunConfiguration::InferFromSettings(
                          settings, asset_manager)
                    : flutter::RunConfiguration::InferFromSettings(settings);

  const char* entrypoint_c = SAFE_ACCESS(info, entrypoint, nullptr);
  const char* library_c = SAFE_ACCESS(info, library_path, nullptr);
  std::string entrypoint_str =
      (entrypoint_c != nullptr && strlen(entrypoint_c) > 0) ? entrypoint_c : "";
  std::string library_str =
      (library_c != nullptr && strlen(library_c) > 0) ? library_c : "";

  if (!entrypoint_str.empty() && !library_str.empty()) {
    run_configuration.SetEntrypointAndLibrary(entrypoint_str, library_str);
  } else if (!entrypoint_str.empty()) {
    run_configuration.SetEntrypoint(entrypoint_str);
  }

  int argc = SAFE_ACCESS(info, entrypoint_argc, 0);
  const char* const* argv = SAFE_ACCESS(info, entrypoint_argv, nullptr);
  if (argc > 0 && argv == nullptr) {
    return LOG_EMBEDDER_ERROR(
        kInvalidArguments,
        "Could not determine Dart entrypoint arguments as entrypoint_argc was "
        "set, but entrypoint_argv was null.");
  }
  if (argc < 0) {
    return LOG_EMBEDDER_ERROR(kInvalidArguments,
  ...
  if (args && SAFE_ACCESS(args, engine_id, 0) != 0) {
    run_configuration.SetEngineId(args->engine_id);
  }

  const char* initial_route_c = SAFE_ACCESS(info, initial_route, nullptr);
  std::string initial_route =
      (initial_route_c != nullptr && strlen(initial_route_c) > 0)
          ? initial_route_c
          : "/";

  std::unique_ptr<flutter::Shell> spawned_shell =
      parent_shell.Spawn(std::move(run_configuration), initial_route,
                         on_create_platform_view, on_create_rasterizer);

  if (!spawned_shell || !spawned_shell->IsSetup()) {
    return LOG_EMBEDDER_ERROR(kInternalInconsistency,
                              "Could not spawn Shell instance.");
  }

  auto spawned_engine = std::make_unique<flutter::EmbedderEngine>(
      parent_engine->GetThreadHost(), parent_engine->GetTaskRunners(),
...
FlutterEngineResult FlutterEngineRunTask(FLUTTER_API_SYMBOL(FlutterEngine)
  ...
  if (task == nullptr) {
    return LOG_EMBEDDER_ERROR(kInvalidArguments, "Invalid task pointer.");
  }
...
FlutterEngineResult FlutterEngineUpdateAssetResolver(
  ...
  if (resolver == nullptr) {
    return LOG_EMBEDDER_ERROR(kInvalidArguments,
                              "Asset resolver pointer was null.");
  }

  if (resolver->struct_size != sizeof(FlutterAssetResolver)) {
    return LOG_EMBEDDER_ERROR(kInvalidArguments,
                              "Asset resolver struct size did not match "
                              "sizeof(FlutterAssetResolver).");
  }

  if (resolver->get_asset_callback == nullptr) {
    return LOG_EMBEDDER_ERROR(kInvalidArguments,
  ...
  if (!embedder_engine->UpdateAssetResolver(std::move(embedder_resolver))) {
    return LOG_EMBEDDER_ERROR(kInternalInconsistency,
                              "Could not update asset resolver on engine.");
  }
...
FlutterEngineResult FlutterEngineLoadDartDeferredLibrary(
  ...
  if (load_info == nullptr) {
    return LOG_EMBEDDER_ERROR(kInvalidArguments,
                              "Invalid load_info specified.");
  }

  if (SAFE_ACCESS(load_info, struct_size, 0) !=
      sizeof(FlutterLoadDeferredLibraryInfo)) {
    return LOG_EMBEDDER_ERROR(kInvalidArguments,
                              "Invalid struct_size specified in load_info.");
  }

  const uint8_t* data = SAFE_ACCESS(load_info, isolate_snapshot_data, nullptr);
  size_t data_size = SAFE_ACCESS(load_info, isolate_snapshot_data_size, 0);

  if (data == nullptr || data_size == 0) {
    return LOG_EMBEDDER_ERROR(
        kInvalidArguments,
        "isolate_snapshot_data was null or isolate_snapshot_data_size was 0.");
  }

  const uint8_t* instructions =
      SAFE_ACCESS(load_info, isolate_snapshot_instructions, nullptr);
  size_t instructions_size =
      SAFE_ACCESS(load_info, isolate_snapshot_instructions_size, 0);

  if ((instructions == nullptr && instructions_size > 0) ||
      (instructions != nullptr && instructions_size == 0)) {
    return LOG_EMBEDDER_ERROR(
        kInvalidArguments,
        "isolate_snapshot_instructions and isolate_snapshot_instructions_size "
        "must either both be valid or both be null/0.");
  }

  auto snapshot_data = std::make_unique<fml::NonOwnedMapping>(data, data_size);

  std::unique_ptr<const fml::Mapping> snapshot_instructions;
  if (instructions != nullptr && instructions_size > 0) {
    snapshot_instructions =
        std::make_unique<fml::NonOwnedMapping>(instructions, instructions_size);
  }

  intptr_t loading_unit_id = SAFE_ACCESS(load_info, loading_unit_id, 0);
  auto engine = reinterpret_cast<flutter::EmbedderEngine*>(raw_engine);
  return engine->LoadDartDeferredLibrary(loading_unit_id,
...
FlutterEngineResult FlutterEngineLoadDartDeferredLibraryError(
  ...
  if (error_info == nullptr) {
    return LOG_EMBEDDER_ERROR(kInvalidArguments,
                              "Invalid error_info specified.");
  }

  if (SAFE_ACCESS(error_info, struct_size, 0) !=
      sizeof(FlutterLoadDeferredLibraryErrorInfo)) {
    return LOG_EMBEDDER_ERROR(kInvalidArguments,
                              "Invalid struct_size specified in error_info.");
  }

  const char* error_message = SAFE_ACCESS(error_info, error_message, nullptr);
  if (error_message == nullptr) {
    return LOG_EMBEDDER_ERROR(kInvalidArguments,
                              "Invalid error_message specified in error_info.");
  }

  intptr_t loading_unit_id = SAFE_ACCESS(error_info, loading_unit_id, 0);
  bool transient = SAFE_ACCESS(error_info, transient, false);

  auto engine = reinterpret_cast<flutter::EmbedderEngine*>(raw_engine);
  return engine->LoadDartDeferredLibraryError(
...
FlutterEngineResult FlutterEngineGetCallbackInformation(
  ...
  if (info_out == nullptr) {
    return LOG_EMBEDDER_ERROR(kInvalidArguments,
                              "Invalid info_out pointer specified.");
  }

  if (SAFE_ACCESS(info_out, struct_size, 0) <
      sizeof(FlutterCallbackInformation)) {
    return LOG_EMBEDDER_ERROR(kInvalidArguments,
                              "Invalid struct_size specified in info_out.");
  }
