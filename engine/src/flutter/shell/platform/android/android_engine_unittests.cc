...
TEST(AndroidShellHolder, SpawnEngineInvalidArguments) {
  ...
  class FakePlatformViewDelegate : public PlatformView::Delegate {
   public:
    ...
    void OnPlatformViewSendViewFocusEvent(
        const ViewFocusEvent& event) override {}
  ...
  class MockPlatformViewDelegate final : public PlatformView {
   public:
    ...
    void SetupImpellerContext() override {
      setup_impeller_context_called = true;
    }
  ...
  TEST_F(PlatformViewAndroidTest, PlatformViewDelegateSeam) {
    ...
    MockPlatformViewDelegate delegate_platform_view(fake_delegate,
                                                    task_runners);

    platform_view->SetPlatformView(&delegate_platform_view);
    EXPECT_EQ(platform_view->GetPlatformViewDelegate(),
              &delegate_platform_view);
  ...
  TEST_F(PlatformViewAndroidTest,
         PlatformViewDelegateMessagingAndDeferredSeam) {
    auto holder = CreateShellHolder();
    ASSERT_NE(holder, nullptr);
    ASSERT_TRUE(holder->IsValid());
    ...
    EXPECT_FALSE(
        delegate_platform_view.load_dart_deferred_library_error_called);
  ...
  TEST_F(PlatformViewAndroidTest,
    ...
    // Fallback calls:
    platform_view->HandlePlatformMessage(
        std::make_unique<flutter::PlatformMessage>("fallback_channel",
                                                   nullptr));
    platform_view->OnPreEngineRestart();
  ...
  // CreateResourceContext, ReleaseResourceContext, and GetImpellerContext
  // forward to the registered PlatformView delegate when present.
  TEST_F(PlatformViewAndroidTest, PlatformViewDelegateSurfaceAndContextSeam) {
    ...
    EXPECT_FALSE(
        delegate_platform_view.create_snapshot_surface_producer_called);
  ...
  TEST_F(PlatformViewAndroidTest,
         PlatformViewDelegateSurfaceAndContextFallback) {
    auto holder = CreateShellHolder();
    ASSERT_NE(holder, nullptr);
    ASSERT_TRUE(holder->IsValid());
  ...
  TEST_F(PlatformViewAndroidTest, SendWindowMetricsEventValidation) {
    ...
    EXPECT_EQ(platform_view->SendWindowMetricsEvent(nullptr),
              kInvalidArguments);
    ...
    EXPECT_EQ(fake_delegate.last_viewport_metrics.physical_view_inset_top,
              10.0);
  ...
  TEST_F(PlatformViewAndroidTest,
         AccessibilityFeaturesDispatchEmbedderApiPath) {
    auto holder = CreateShellHolder();
    ASSERT_NE(holder, nullptr);
  ...
  TEST_F(PlatformViewAndroidTest,
         RegisterImageExternalTextureInvalidArguments) {
    auto holder = CreateShellHolder();
    ASSERT_NE(holder, nullptr);
  ...
  TEST_F(PlatformViewAndroidTest, DisplayPlatformViewDirectSeam) {
    ...
    EXPECT_EQ(platform_view->DisplayPlatformView(3, 10, 20, 100, 200, 300, 400,
                                                 stack1),
              kSuccess);

    MutatorsStack stack2;
    EXPECT_EQ(platform_view->DisplayPlatformViewEmbedder(3, 10, 20, 100, 200,
                                                         300, 400, stack2),
              kSuccess);
  }
  ...
  TEST_F(PlatformViewAndroidTest, DisplayOverlaySurfaceDirectSeam) {
    ...
    EXPECT_EQ(platform_view->DisplayOverlaySurface(30, 5, 15, 50, 60),
              kSuccess);
  }
  ...
  TEST_F(PlatformViewAndroidTest, DisplayVirtualDisplayPlatformViewDirectSeam) {
    ...
    EXPECT_EQ(
        ...
        kSuccess);
    EXPECT_EQ(platform_view->DisplayVirtualDisplayPlatformViewEmbedder(
                  3, 10, 20, 100, 200),
              kSuccess);
  }
  ...
  TEST_F(PlatformViewAndroidTest,
    ...
    EXPECT_EQ(
        ...
        kInvalidArguments);
    EXPECT_EQ(
        platform_view->DisplayVirtualDisplayPlatformView(1, 0, 0, -1, 100),
        kInvalidArguments);
    EXPECT_EQ(
        platform_view->DisplayVirtualDisplayPlatformView(1, 0, 0, 100, -1),
        kInvalidArguments);
  }

  TEST_F(PlatformViewAndroidTest, DisplayPlatformView2EmbedderApiPath) {
    auto jni = std::make_shared<JNIMock>();
    EXPECT_CALL(*jni, onDisplayPlatformView2(2, 10, 20, 100, 200, 300, 400,
                                             ::testing::_))
        .Times(1);
  ...
  TEST_F(PlatformViewAndroidTest, DisplayPlatformView2DirectSeam) {
    auto jni = std::make_shared<JNIMock>();
    EXPECT_CALL(*jni, onDisplayPlatformView2(3, 10, 20, 100, 200, 300, 400,
                                             ::testing::_))
        .Times(2);
  ...
  TEST_F(PlatformViewAndroidTest, DisplayPlatformView2InvalidArguments) {
    ...
    MutatorsStack stack;
    EXPECT_EQ(platform_view->DisplayPlatformView2(-1, 0, 0, 100, 100, 100, 100,
                                                  stack),
              kInvalidArguments);
    EXPECT_EQ(
