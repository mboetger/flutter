...
//------------------------------------------------------------------------------
/// @brief      Manages Android platform interaction, surface lifecycle,
///             external textures, and JNI bridging.
///
///             `PlatformViewAndroid` no longer inherits directly from
///             `flutter::PlatformView`. Instead, all engine interactions are
///             forwarded via `PlatformViewAndroid::Delegate`.
///
class PlatformViewAndroid {
 public:
  //----------------------------------------------------------------------------
  /// @brief      Delegate interface through which `PlatformViewAndroid`
  ///             communicates with the Flutter engine shell or adapter.
  ///
  class Delegate {
   public:
    virtual ~Delegate() = default;

    virtual const flutter::TaskRunners& GetTaskRunners() const = 0;
    virtual const flutter::Settings& OnPlatformViewGetSettings() const = 0;
    ...
    virtual void OnPlatformViewSemanticsAction(int64_t view_id,
                                               int32_t node_id,
 ...
 private:
  PlatformViewAndroid::Delegate& delegate_;
  ...
  std::shared_ptr<flutter::Texture> CreateSurfaceTextureExternalTexture(
      int64_t texture_id,
      const fml::jni::ScopedJavaGlobalRef<jobject>& surface_texture);

  std::shared_ptr<flutter::Texture> CreateImageExternalTexture(
      int64_t texture_id,
      const fml::jni::ScopedJavaGlobalRef<jobject>& image_texture_entry,
      ImageExternalTexture::ImageLifecycle lifecycle);
