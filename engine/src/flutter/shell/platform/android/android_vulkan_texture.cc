...
void DefaultAndroidVulkanTextureProvider::EnsureLoaded() const {
  ...
  libvulkan_ = library_loader_->LoadLibrary("libvulkan.so");
  ...
  if (ycbcr_desc != nullptr) {
    ...
    vk_desc.ycbcr_conversion = *ycbcr_desc;
                 ...
                 static_cast<uint32_t>(
                     AndroidHardwareBufferFormat::kY8Cb8Cr8_420) ||
             ahb_desc.format == static_cast<uint32_t>(
                                    AndroidHardwareBufferFormat::kYCbCr_P010) ||
             ahb_desc.format == static_cast<uint32_t>(
                                    AndroidHardwareBufferFormat::kYCbCr_P210)) {
