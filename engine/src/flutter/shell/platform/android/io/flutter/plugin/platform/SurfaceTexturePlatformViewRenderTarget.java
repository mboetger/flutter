...
public class SurfaceTexturePlatformViewRenderTarget implements PlatformViewRenderTarget {
  ...
  private boolean shouldRecreateSurface = false;
  private final TextureRegistry.OnTrimMemoryListener trimMemoryListener =
      new TextureRegistry.OnTrimMemoryListener() {
        @Override
        public void onTrimMemory(int level) {
          // When a memory pressure warning is received and the level is greater than or equal to
          // {@code ComponentCallbacks2.TRIM_MEMORY_BACKGROUND}, the Android system releases the
          // underlying surface. If we continue to use the surface (e.g., call lockHardwareCanvas),
          // a crash occurs, and we found that this crash appeared on Android 10 and above.
          ...
          if (level >= TRIM_MEMORY_BACKGROUND && Build.VERSION.SDK_INT >= API_LEVELS.API_29) {
  ...
  private void recreateSurfaceIfNeeded() {
    if (surface != null && !shouldRecreateSurface) {
