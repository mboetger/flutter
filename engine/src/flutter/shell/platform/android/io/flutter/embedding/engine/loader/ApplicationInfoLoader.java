...
public final class ApplicationInfoLoader {
    ...
    try {
      ...
      while (eventType != XmlResourceParser.END_DOCUMENT) {
        if (eventType == XmlResourceParser.START_TAG) {
          ...
          } else if (xrp.getName().equals("base-config")) {
            parseBaseConfig(xrp, output);
          }
        }
        eventType = xrp.next();
      }
  ...
  private static void parseBaseConfig(XmlResourceParser xrp, JSONArray output)
      throws IOException, XmlPullParserException {
    boolean cleartextTrafficPermitted =
        xrp.getAttributeBooleanValue(null, "cleartextTrafficPermitted", false);
    if (cleartextTrafficPermitted) {
      JSONArray outputArray = new JSONArray();
      outputArray.put("*");
      outputArray.put(true);
      outputArray.put(true);
  ...
  private static void parseDomain(
    ...
    if (!android.util.Patterns.IP_ADDRESS.matcher(domain).matches()) {
      JSONArray outputArray = new JSONArray();
