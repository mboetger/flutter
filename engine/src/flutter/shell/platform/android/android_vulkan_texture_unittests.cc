...
TEST(AndroidVulkanTextureTest, FormatAndLayoutEnums) {
  ...
  EXPECT_EQ(static_cast<uint32_t>(AndroidVulkanFormat::kR8G8B8A8_UNORM), 37u);
  EXPECT_EQ(static_cast<uint32_t>(AndroidVulkanFormat::kR8G8B8A8_SRGB), 43u);
  EXPECT_EQ(static_cast<uint32_t>(AndroidVulkanFormat::kB8G8R8A8_UNORM), 44u);
  EXPECT_EQ(static_cast<uint32_t>(AndroidVulkanFormat::kB8G8R8A8_SRGB), 50u);
  EXPECT_EQ(static_cast<uint32_t>(AndroidVulkanFormat::kR16G16B16A16_SFLOAT),
            97u);
  EXPECT_EQ(
      static_cast<uint32_t>(AndroidVulkanFormat::kG8_B8_R8_3PLANE_420_UNORM),
      1000156000u);
  EXPECT_EQ(
      static_cast<uint32_t>(AndroidVulkanFormat::kG8_B8R8_2PLANE_420_UNORM),
      1000156001u);
...
TEST(AndroidVulkanTextureTest, ImageDescValidationAndEquality) {
  ...
  EXPECT_EQ(desc_rgba.format,
            static_cast<uint32_t>(AndroidVulkanFormat::kR8G8B8A8_UNORM));
  EXPECT_EQ(
  ...
  EXPECT_TRUE(desc_ycbcr.ycbcr_conversion.has_value());
...
TEST(AndroidVulkanTextureTest, InMemoryVulkanExternalTextureLifecycle) {
  ...
  EXPECT_EQ(ext_texture.destruction_callback, destruction_cb);
...
TEST(AndroidVulkanTextureTest, CreateFromAHardwareBufferIntegration) {
  ...
  ahb_yuv_desc.format =
      static_cast<uint32_t>(AndroidHardwareBufferFormat::kY8Cb8Cr8_420);
