...
DefaultOSLibrary::DefaultOSLibrary(std::string name)
    : name_(std::move(name)), owns_handle_(true) {
...
std::shared_ptr<OSLibrary> DefaultOSLibraryLoader::LoadLibrary(
    const char* library_name) {
  TRACE_EVENT1("flutter", "DefaultOSLibraryLoader::LoadLibrary", "name",
...
std::shared_ptr<OSLibrary> MockOSLibraryLoader::LoadLibrary(
    const char* library_name) {
  TRACE_EVENT1("flutter", "MockOSLibraryLoader::LoadLibrary", "name",
               library_name ? library_name : "<null>");
  if (!library_name) {
    return nullptr;
  }
