...
TEST(OSLibraryLoaderTest, DefaultOSLibraryLoaderMissingLibraryFallback) {
  ...
  auto lib = loader->LoadLibrary("lib_nonexistent_dummy_android_lib.so");
  ...
  EXPECT_EQ(loader->LoadLibrary(nullptr), nullptr);
...
TEST(PlatformViewsTest, AndroidPlatformViewsControllerIntegration) {
  ...
  ASSERT_TRUE(overlay_id.has_value());
  EXPECT_TRUE(controller.OnDisplayOverlaySurface(*overlay_id, 0, 0, 640, 480));
...
TEST(GlobalVMInitializationTest, JniDelegateVMOperations) {
  ...
  EXPECT_TRUE(delegate->GetVMArgs().has_value());
  EXPECT_EQ(delegate->GetVMArgs().value(), args);
