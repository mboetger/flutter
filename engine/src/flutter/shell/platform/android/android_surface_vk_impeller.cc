...
std::unique_ptr<Surface> AndroidSurfaceVKImpeller::CreateGPUSurface(
  ...
  if (!IsValid()) {
    FML_LOG(ERROR)
        << "AndroidSurfaceVKImpeller::CreateGPUSurface: IsValid() is false";
    return nullptr;
  }

  FML_LOG(ERROR)
      << "AndroidSurfaceVKImpeller::CreateGPUSurface: native_window_ is "
      << (native_window_ ? "not null" : "null");
  if (native_window_) {
    FML_LOG(ERROR) << "AndroidSurfaceVKImpeller::CreateGPUSurface: "
  ...
  FML_LOG(ERROR)
      << "AndroidSurfaceVKImpeller::CreateGPUSurface: gpu_surface->IsValid() = "
      << gpu_surface->IsValid();
...
bool AndroidSurfaceVKImpeller::SetNativeWindow(
  ...
  if (window && (native_window_ == window)) {
    FML_LOG(ERROR) << "AndroidSurfaceVKImpeller::SetNativeWindow: window is "
                      "same as native_window_";
