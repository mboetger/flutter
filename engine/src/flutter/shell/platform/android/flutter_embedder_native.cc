...
bool FlutterEmbedderNative::RegisterHardwareBufferTexture(
    int64_t texture_id,
    std::shared_ptr<AndroidHardwareBuffer> initial_buffer) const {
  TRACE_EVENT1("flutter",
  ...
  if (registered && initial_buffer) {
    jni_router_->RouteSetHardwareBufferFrame(texture_id,
                                             std::move(initial_buffer));
  }
  return registered;
}
...
bool FlutterEmbedderNative::SetHardwareBufferFrame(
    int64_t texture_id,
    std::shared_ptr<AndroidHardwareBuffer> buffer) const {
  TRACE_EVENT1("flutter",
  ...
  return jni_router_->RouteSetHardwareBufferFrame(texture_id,
                                                  std::move(buffer));
}
...
bool FlutterEmbedderNative::RegisterVulkanTexture(
    int64_t texture_id,
    std::shared_ptr<AndroidVulkanExternalTexture> initial_texture) const {
  ...
  if (registered && initial_texture) {
    jni_router_->RouteSetVulkanTextureFrame(texture_id,
                                            std::move(initial_texture));
  }
  return registered;
}
...
bool FlutterEmbedderNative::SetVulkanTextureFrame(
    int64_t texture_id,
    std::shared_ptr<AndroidVulkanExternalTexture> texture) const {
  TRACE_EVENT1("flutter",
  ...
  return jni_router_->RouteSetVulkanTextureFrame(texture_id,
                                                 std::move(texture));
}
