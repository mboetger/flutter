...
TEST(AndroidSemanticsTest, EncodeFlagsComprehensive) {
  ...
  FlutterSemanticsFlag dep_flag = static_cast<FlutterSemanticsFlag>(
      kFlutterSemanticsFlagIsButton | kFlutterSemanticsFlagIsTextField);
  int64_t dep_res = AndroidSemanticsMapper::EncodeFlags(nullptr, dep_flag);
  EXPECT_EQ(dep_res, (1 << 3) | (1 << 4));
...
TEST(AndroidSemanticsTest, SemanticsNodePackingAndBinaryVerification) {
  ...
  FlutterSemanticsNode2 node = {
      ...
      .actions = static_cast<FlutterSemanticsAction>(
          kFlutterSemanticsActionTap | kFlutterSemanticsActionFocus),
  ...
  EXPECT_EQ(buf32[p++],
            static_cast<int32_t>(kFlutterSemanticsActionTap |
                                 kFlutterSemanticsActionFocus));  // actions
