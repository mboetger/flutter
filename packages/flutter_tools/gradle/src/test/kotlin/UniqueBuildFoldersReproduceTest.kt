...
class UniqueBuildFoldersReproduceTest {
    @Test
    fun `copyFlutterAssets task output directory is unique and does not conflict with mergeAssets`(
        ...
        every { mockAndroidSourceSet.assets.srcDir(any()) } returns mockAndroidSourceDirectorySet
        ...
        val mockMergeAssetsOutputDirProperty = mockk<org.gradle.api.file.DirectoryProperty>(relaxed = true)
        every { mockMergeAssets.outputDir } returns mockMergeAssetsOutputDirProperty
        ...
        // Actual task that should be captured to test
        ...
        val intoDirs = mutableListOf<Any>()
        verify {
            copyTask.into(capture(intoDirs))
        }
        
        // Assert that none of the destination directories configured on the copy task is the mergeAssets output directory.
        intoDirs.forEach { intoDir ->
            assertNotEquals(mockMergeAssetsOutputDirProperty, intoDir)
        }
    }
