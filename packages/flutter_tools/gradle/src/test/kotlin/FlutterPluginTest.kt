...
class FlutterPluginTest {
    ...
    @Test
    fun `aligns Kotlin JVM target with Java target compatibility`(
        @TempDir tempDir: Path
    ) {
        val projectDir = tempDir.resolve("project-dir").resolve("android").resolve("app")
        projectDir.toFile().mkdirs()
        ...
        val mockBaseExtension = mockk<BaseExtension>(relaxed = true)
        every { project.extensions.findByType(BaseExtension::class.java) } returns mockBaseExtension
        every { mockBaseExtension.compileOptions.targetCompatibility } returns org.gradle.api.JavaVersion.VERSION_11
        ...
        val mockExtraProperties = mockk<ExtraPropertiesExtension>(relaxed = true)
        every { project.extensions.findByName("ext") } returns mockExtraProperties
        every { mockExtraProperties.has("flutterKotlinJvmTargetAligned") } returns false

        // Setup allprojects to return our project
        every { project.allprojects } returns setOf(project)

        // Mock KotlinCompile task and its kotlinOptions
        val mockKotlinCompile = mockk<KotlinCompile>(relaxed = true)
        val mockKotlinOptions = mockk<org.jetbrains.kotlin.gradle.dsl.KotlinJvmOptions>(relaxed = true)
        every { mockKotlinCompile.kotlinOptions } returns mockKotlinOptions

        // Mock tasks.withType(KotlinCompile::class.java)
        val mockTaskCollection = mockk<org.gradle.api.tasks.TaskCollection<KotlinCompile>>(relaxed = true)
        every { project.tasks.withType(KotlinCompile::class.java) } returns mockTaskCollection

        // Capture and execute the configureEach action immediately on the mock task
        val actionSlot = slot<Action<KotlinCompile>>()
        every { mockTaskCollection.configureEach(capture(actionSlot)) } answers {
            actionSlot.captured.execute(mockKotlinCompile)
        }
        ...
        // Verify that the jvmTarget was set to "11"
        verify(exactly = 1) {
            mockKotlinOptions.jvmTarget = "11"
        }
    }
