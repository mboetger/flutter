...
void main() {
  ...
  group('doctor', () {
    group('device validator', () {
      ...
      testWithoutContext('adb failure does not crash Connected device check', () async {
        ...
        final deviceManager = _TestDeviceManager(
          logger: BufferLogger.test(),
          deviceDiscoverers: <DeviceDiscovery>[androidDevices],
        );
        final deviceValidator = DeviceValidator(deviceManager: deviceManager);

        final ValidationResult result = await deviceValidator.validate();
        expect(result.type, isNot(ValidationType.crash));
      });
    });
  });
...
class _TestAndroidSdk extends Fake implements AndroidSdk {
  @override
  String? get adbPath => 'adb';
}

class _TestDeviceManager extends DeviceManager {
  _TestDeviceManager({required super.logger, required this.deviceDiscoverers});
