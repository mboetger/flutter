...
void main() {
  ...
  group('daemon', () {
    ...
    testUsingContext(
      'device.enable does not terminate daemon when device discovery throws',
      () async {
        daemon = Daemon(daemonConnection, notifyingLogger: notifyingLogger);
        final discoverer = ThrowingPollingDeviceDiscovery();
        daemon.deviceDomain.addDeviceDiscoverer(discoverer);
        daemonStreams.inputs.add(
          DaemonMessage(<String, Object?>{'id': 0, 'method': 'device.enable'}),
        );
        final DaemonMessage response = await daemonStreams.outputs.stream.firstWhere(_notEvent);
        expect(response.data['id'], 0);

        await discoverer.polledCompleter.future;
      },
    );
...
class ThrowingPollingDeviceDiscovery extends PollingDeviceDiscovery {
  ...
  final Completer<void> polledCompleter = Completer<void>();
  ...
  Future<List<Device>> pollingGetDevices({
    ...
    if (!polledCompleter.isCompleted) {
      polledCompleter.complete();
    }
    throwToolExit('fake-tool-exit');
  }
