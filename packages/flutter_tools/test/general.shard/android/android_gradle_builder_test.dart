...
[
  {
    ...
    testUsingContext(
      'gradlew build succeeds if project path contains ampersand on Windows',
      () async {
        final Directory directory = fileSystem.directory(r'C:\Flutter&Dart\projects\start_app');
        final String gradlewPath = fileSystem.path.join(directory.path, 'android', 'gradlew.bat');
        ...
        final builder = AndroidGradleBuilder(
          ...
          artifacts: Artifacts.test(),
          analytics: fakeAnalytics,
          gradleUtils: FakeGradleUtilsWithAmpersand(gradlewPath),
          platform: FakePlatform(operatingSystem: 'windows'),
          androidStudio: FakeAndroidStudio(),
        );

        directory.childDirectory('android').childFile('build.gradle').createSync(recursive: true);
        directory
        ...
        directory.childDirectory('android').childDirectory('app').childFile('build.gradle')
          ..createSync(recursive: true)
          ..writeAsStringSync('apply from: "path/to/flutter.gradle"');
        directory
            .childDirectory('android')
            .childDirectory('app')
            .childFile('src/main/AndroidManifest.xml')
          ..createSync(recursive: true)
          ..writeAsStringSync(minimalV2EmbeddingManifest);
        ...
        final FlutterProject project = FlutterProject.fromDirectoryTest(directory);
...
class FakeGradleUtilsWithAmpersand extends Fake implements GradleUtils {
  FakeGradleUtilsWithAmpersand(this.executablePath);
