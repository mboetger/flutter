...
''');
    ...
    testUsingContext('buildGradleApp validates launch activity presence in APK', () async {
      final builder = AndroidGradleBuilder(
        java: FakeJava(),
        logger: logger,
        processManager: processManager,
      ...
      // 1. Success case: MainActivity is present in DEX
      processManager.addCommand(
        const FakeCommand(
          command: <String>[
            'gradlew',
      ...
      final archiveWithActivity = Archive()
        ..addFile(ArchiveFile('classes.dex', 100, utf8.encode('Lcom/example/MainActivity;')));
      apkFile.writeAsBytesSync(ZipEncoder().encode(archiveWithActivity)!);
...
''');
      ...
      // 2. Failure case: MainActivity is missing from DEX
      processManager.addCommand(
        const FakeCommand(
          command: <String>[
            'gradlew',
      ...
      final archiveWithoutActivity = Archive()
        ..addFile(ArchiveFile('classes.dex', 100, utf8.encode('Lcom/example/OtherClass;')));
      apkFile.writeAsBytesSync(ZipEncoder().encode(archiveWithoutActivity)!);
      ...
      await expectLater(
        ...
        throwsToolExit(
          message:
              'The launch activity class "com.example.MainActivity" was not found in the built APK or Bundle.',
        ),
      );

      // 3. Framework activity case: android.app.NativeActivity (should be skipped and succeed)
      processManager.addCommand(
        const FakeCommand(
          command: <String>[
            'gradlew',
      ...
      project.android.appManifestFile.writeAsStringSync(r'''
<manifest xmlns:android="http://schemas.android.com/apk/res/android" package="com.example">
    <application>
        <meta-data
            android:name="flutterEmbedding"
...
''');
      ...
      // Even though DEX doesn't have it, it should succeed
      await builder.buildGradleApp(
        project: project,
        androidBuildInfo: const AndroidBuildInfo(
          BuildInfo(
      ...
      // 4. No leading dot case: MainActivity (should resolve to com.example.MainActivity and succeed)
      processManager.addCommand(
        const FakeCommand(
          command: <String>[
            'gradlew',
...
''');
      ...
      final archiveWithActivityNoDot = Archive()
        ..addFile(ArchiveFile('classes.dex', 100, utf8.encode('Lcom/example/MainActivity;')));
      apkFile.writeAsBytesSync(ZipEncoder().encode(archiveWithActivityNoDot)!);
      ...
      // 5. Placeholder case: ${launchActivity} (should be skipped and succeed)
      processManager.addCommand(
        const FakeCommand(
          command: <String>[
            'gradlew',
...
''');
      ...
      // Write an empty DEX, which would normally fail validation, but it should be skipped
      final archiveEmpty = Archive()
        ..addFile(ArchiveFile('classes.dex', 100, utf8.encode('Lcom/example/OtherClass;')));
      apkFile.writeAsBytesSync(ZipEncoder().encode(archiveEmpty)!);
