...
void main() {
  group('gradle build', () {
    ...
    testUsingContext('AndroidGradleBuilder serializes concurrent buildApk calls', () {
      fakeAsync((FakeAsync async) {
        final trackingProcessManager = ConcurrencyTrackingProcessManager();
        final builder = AndroidGradleBuilder(
          java: FakeJava(),
          logger: logger,
          processManager: trackingProcessManager,
          fileSystem: fileSystem,
          artifacts: Artifacts.test(),
          analytics: fakeAnalytics,
          gradleUtils: FakeGradleUtils(),
        ...
        // We expect two assembleDebug commands to be run.
        // We add delays in the commands to ensure they would overlap if not serialized.
        ...
        trackingProcessManager.addCommand(command);
        trackingProcessManager.addCommand(command);
        ...
        const androidBuildInfo = AndroidBuildInfo(
          BuildInfo(
            BuildMode.debug,
            null,
            treeShakeIcons: false,
        ...
        // Launch both builds concurrently.
        var completed = false;
        ...
        // Elapse enough simulated time for both builds to complete.
        async.elapse(const Duration(seconds: 20));

        // Verify completion and serialization.
        expect(completed, isTrue);
        expect(trackingProcessManager.maxConcurrentProcesses, equals(1));
      });
    }, overrides: <Type, Generator>{AndroidStudio: () => FakeAndroidStudio()});
...
class ConcurrencyTrackingProcessManager implements ProcessManager {
  final FakeProcessManager _delegate = FakeProcessManager.empty();

  int activeProcesses = 0;
  int maxConcurrentProcesses = 0;

  void addCommand(FakeCommand command) => _delegate.addCommand(command);
  ...
  Future<io.Process> start(
    ...
    final io.Process process = await _delegate.start(
      command,
