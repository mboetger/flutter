...
Gradle Crashed
    ...
    testUsingContext(
      'buildAar with output-dir skips host/outputs/repo directory structure',
      () async {
        final builder = AndroidGradleBuilder(
          java: FakeJava(),
          logger: logger,
        ...
        fileSystem.file('.android/gradle.properties').writeAsStringSync('irrelevant');
        fileSystem.file('.android/build.gradle').createSync(recursive: true);
        fileSystem.directory('/custom/repo').createSync(recursive: true);

        await builder.buildAar(
          project: FlutterProject.fromDirectoryTest(fileSystem.currentDirectory),
          outputDirectoryPath: '/custom/repo',
          androidBuildInfo: const <AndroidBuildInfo>{
            AndroidBuildInfo(
              BuildInfo(
                BuildMode.debug,
