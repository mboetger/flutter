...
''');
    ...
    testUsingContext(
      'incompatibleKotlinVersionHandler is ignored if KGP mismatch printed before What went wrong',
      () async {
        final builder = AndroidGradleBuilder(
          java: FakeJava(),
          logger: logger,
        ...
        await expectLater(() async {
          await builder.buildGradleApp(
            ...
            isBuildingBundle: false,
            configOnly: false,
            localGradleErrors: <GradleHandledError>[incompatibleKotlinVersionHandler],
          );
        }, throwsToolExit(message: 'Gradle task assembleRelease failed with exit code 1'));

        expect(
          logger.statusText,
          isNot(contains('Your project requires a newer version of the Kotlin Gradle plugin.')),
        );
      },
      overrides: <Type, Generator>{
        AndroidStudio: () => FakeAndroidStudio(),
    ...
    testUsingContext(
      'incompatibleKotlinVersionHandler triggers if KGP mismatch printed after What went wrong',
      () async {
        final builder = AndroidGradleBuilder(
          java: FakeJava(),
          logger: logger,
        ...
        expect(
          logger.statusText,
          contains('Your project requires a newer version of the Kotlin Gradle plugin.'),
        );
      },
      overrides: <Type, Generator>{
        AndroidStudio: () => FakeAndroidStudio(),
