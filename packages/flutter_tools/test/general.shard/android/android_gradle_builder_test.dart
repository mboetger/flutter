...
''');
    ...
    group('x86 ABI mismatch checks', () {
      ...
      testUsingContext(
        'warns when x86 ABI contains native libs but is missing libflutter.so in release',
        () async {
          final builder = AndroidGradleBuilder(
            java: FakeJava(),
            logger: logger,
          ...
          // Create a mismatched zip archive
          final archive = Archive()
            ..addFile(ArchiveFile('lib/arm64-v8a/libapp.so', 50, List<int>.filled(50, 0)))
            ..addFile(ArchiveFile('lib/arm64-v8a/libflutter.so', 50, List<int>.filled(50, 0)))
            ..addFile(
              ArchiveFile('lib/x86/libapp.so', 50, List<int>.filled(50, 0)),
            ); // Missing libflutter.so for x86!
          ...
          expect(
            logger.warningText,
            contains(
              'Warning: Your APK contains native libraries for the following Intel architectures: x86.',
            ),
          );
          expect(processManager, hasNoRemainingExpectations);
        },
      ...
      testUsingContext('does not warn when no Intel ABIs are present', () async {
        final builder = AndroidGradleBuilder(
          java: FakeJava(),
          logger: logger,
          processManager: processManager,
