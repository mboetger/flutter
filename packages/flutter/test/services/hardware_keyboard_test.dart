...
void main() {
  ...
  testWidgets('Instantly dispatch synthesized key events when the queue is empty', (
    WidgetTester tester,
  ) async {
    final focusNode = FocusNode();
    addTearDown(focusNode.dispose);
  ...
  testWidgets('Postpone synthesized key events when the queue is not empty', (
    WidgetTester tester,
  ) async {
  ...
  testWidgets('Exceptions from HardwareKeyboard handlers are caught and reported', (
    WidgetTester tester,
  ) async {
  ...
  testWidgets('Irregular key events are processed', (WidgetTester tester) async {
    final logs = <String>[];
  ...
  // Regression test for https://github.com/flutter/flutter/issues/95630
  testWidgets(
    'Null check operator used on a null value in _convertRawEventAndStore when modifier is synchronized/removed',
    (WidgetTester tester) async {
      final events = <KeyEvent>[];
      HardwareKeyboard.instance.addHandler((KeyEvent event) {
        events.add(event);
      ...
      // 1. Send KeyDownEvent for ShiftLeft.
      final Map<String, dynamic> shiftDownMessage = KeyEventSimulator.getKeyData(
        LogicalKeyboardKey.shiftLeft,
        platform: 'android',
      );
      shiftDownMessage['metaState'] =
          RawKeyEventDataAndroid.modifierLeftShift | RawKeyEventDataAndroid.modifierShift;
      await tester.binding.keyEventManager.handleRawKeyMessage(shiftDownMessage);

      expect(
        HardwareKeyboard.instance.physicalKeysPressed,
        contains(PhysicalKeyboardKey.shiftLeft),
      );
      expect(RawKeyboard.instance.physicalKeysPressed, contains(PhysicalKeyboardKey.shiftLeft));

      // Clear events list to focus on the next event's results.
      events.clear();

      // 2. Send KeyDownEvent for KeyA with metaState = 0 (ShiftLeft is not pressed anymore).
      // In the buggy code, this will trigger the null-check crash.
      final Map<String, dynamic> keyADownMessage = KeyEventSimulator.getKeyData(
        LogicalKeyboardKey.keyA,
        platform: 'android',
      );
      keyADownMessage['metaState'] = 0;

      await tester.binding.keyEventManager.handleRawKeyMessage(keyADownMessage);

      // In the fixed version, we expect both events to have been successfully processed:
      // A synthesized KeyUpEvent for ShiftLeft, and a KeyDownEvent for KeyA.
      expect(events, hasLength(2));
      expect(events[0], isA<KeyUpEvent>());
      expect(events[0].physicalKey, PhysicalKeyboardKey.shiftLeft);
      expect(events[0].synthesized, isTrue);

      expect(events[1], isA<KeyDownEvent>());
      expect(events[1].physicalKey, PhysicalKeyboardKey.keyA);
      expect(events[1].synthesized, isFalse);
    },
  );
}
